...
// 1. DATA CADANGAN UNTUK DI HOSTING
const DATA_CADANGAN = [];
...
// RUTING UTAMA UNTUK MEMBUKA DASHBOARD (LANGKAH 5)
app.get('/', (req, res) => {
...
app.get('/api/analytics/kategori', async (req, res) => {
    if (isDbConnected) {
        const data = await db.collection('transaksi').aggregate([
            { $group: { _id: "$produk.kategori", total: { $sum: "$pembayaran.total_bayar" } } },
            { $sort: { total: -1 } }
        ]).toArray();
        ...
        res.json(Object.keys(hasil).map(k => ({ _id: k, total: hasil[k] })));
    }
});
...
app.get('/api/analytics/pembayaran', async (req, res) => {
    if (isDbConnected) {
        const data = await db.collection('transaksi').aggregate([
            { $group: { _id: "$pembayaran.metode", total_transaksi: { $sum: 1 } } }
        ]).toArray();
        return res.json(data);
    } else {
        const hasil = {};
        ...
        res.json(Object.keys(hasil).map(k => ({ _id: k, total_transaksi: hasil[k] })));
    }
});
...
app.get('/api/analytics/tren', async (req, res) => {
    if (isDbConnected) {
        const data = await db.collection('transaksi').aggregate([
            ...
            { $group: { _id: "$bulan", total: { $sum: "$total" } } },
            { $sort: { _id: 1 } }
        ]).toArray();
        return res.json(data);
    } else {
        ...
        res.json(Object.keys(hasil).sort().map(k => ({ _id: k, total: hasil[k] })));
    }
});
...
app.get('/api/analytics/kota', async (req, res) => {
    if (isDbConnected) {
        const data = await db.collection('transaksi').aggregate([
            { $group: { _id: "$lokasi.kota", total: { $sum: "$pembayaran.total_bayar" } } },
            { $sort: { total: -1 } },
            { $limit: 5 }
        ]).toArray();
        return res.json(data);
        ...
        const urut = Object.keys(hasil).map(k => ({ _id: k, total: hasil[k] })).sort((a,b) => b.total - a.total);
...
app.get('/api/analytics/pelanggan', async (req, res) => {
    if (isDbConnected) {
        const data = await db.collection('transaksi').aggregate([
            { $group: { _id: "$pelanggan.tipe", total_omset: { $sum: "$pembayaran.total_bayar" } } }
        ]).toArray();
        return res.json(data);
    } else {
        const hasil = {};
        DATA_CADANGAN.forEach(d => {
            hasil[d.pelanggan.tipe] = (hasil[d.pelanggan.tipe] || 0) + d.pembayaran.total_bayar;
        });
        res.json(Object.keys(hasil).map(k => ({ _id: k, total_omset: hasil[k] })));
    }
});
